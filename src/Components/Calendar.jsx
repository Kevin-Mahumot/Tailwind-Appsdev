...
const Calendar = () => {
  ...
  const upcomingEvent = events.find((event) => moment().isBefore(moment(event.start)));

  // Check if there is a current event
  const currentEvent = events.find((event) => moment().isBetween(moment(event.start), moment(event.end)));
      ...
      <div className='bg-gray-300 px-5 py-4 rounded'>
        ...
        >
          <div className='flex justify-end'>
            <button className='text-blue-500 hover:underline focus:outline-none' onClick={closeModal}>
              Close
            </button>
          </div>
          <div className='mt-4'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4 '>
         ...
         <p className='text-gray-700'>No upcoming events</p>
          ...
          <div className='bg-white p-4 rounded shadow-md hover:shadow-lg'>
            ...
            {currentEvent ? (
              <p className='text-gray-700'>
                {currentEvent.title} - {moment(currentEvent.start).format('MMMM D, YYYY h:mm A')} to{' '}
